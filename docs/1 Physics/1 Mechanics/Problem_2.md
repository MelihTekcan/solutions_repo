...
**Test**: With $$ A = 5 $$, $$ \omega = 1.5 $$, chaos emerges—try it in the simulation!
...
Let’s simulate this with Python using **SciPy’s solve_ivp** for numerical solutions. The code below plots the angle over time and a phase diagram (angle vs. velocity).
...
# Experiment: Try chaotic parameters
A_chaos = 5.0
sol_chaos = solve_ivp(forced_damped_pendulum, t_span, y0, t_eval=t_eval, args=(b, m, g, L, A_chaos, omega))
plt.plot(sol_chaos.t, sol_chaos.y[0])
plt.xlabel("Time (s)")
plt.ylabel("Angle (rad)")
plt.title("Chaotic Motion (A = 5.0)")
plt.show()