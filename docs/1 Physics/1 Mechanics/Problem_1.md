...
<details>
<summary>Click to see simulation code</summary>
...
def simulate_projectile(v0=20, theta_deg=45, g=9.8):
    ...
    plt.plot(x, y)
    plt.title(f'Projectile Motion (v0={v0} m/s, θ={theta_deg}°)')
...
v0_slider = widgets.FloatSlider(value=20, min=5, max=50, description='Initial Velocity (m/s)')
theta_slider = widgets.FloatSlider(value=45, min=0, max=90, description='Angle (degrees)')
g_slider = widgets.FloatSlider(value=9.8, min=5, max=15, description='Gravity (m/s²)')

# Create interactive plot
widgets.interactive(simulate_projectile, v0=v0_slider, theta_deg=theta_slider, g=g_slider)
```
</details>

![Projectile Motion Simulation Output](path_to_simulation_output.png)
